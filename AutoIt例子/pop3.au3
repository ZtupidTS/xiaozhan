...
;---------------------------------------------------------------------------------------------------------------------------
; ?基本职能AU3脚本的基础上， 1939年的RFC 。 
; ?见http://www.ietf.org/rfc/rfc1939.txt 
; ?包含版本： 0.99 （ 2006年3月，第9次） 。 
; ?需要AU3测试版3.1.1.110或更高版本。 
; ?作者：吕克HENNINOT <lhenninot@nordnet.fr> 
;------------------------------------------------- ---------- 
; ?诠释_pop3Connect （服务器，登录，密码， [端口] ） ; Conects向根据POP3服务器。返回0的行， -1 ，如果错误，并提出@错误。服务器可以是一个IP地址，或全文名称。 
; ?字符串_Pop3Dele （ msg_number ） ;删除味邮件msg_number 。返回服务器响应或-1如果错误，并提出@错误。 
; ?阵列_Pop3List （ [ msg_number ] ） ;返回一个数组与邮件和大小（字节） ， -1 ，如果错误答集@错误。 
; ?字符串_Pop3Noop （ ） ;实际上，没有采取任何行动。最有趣的命令符合RFC 1939年; ） 
; ?字符串_Pop3Quit （ ） ;验证您的行动（例如缺失）和站连接，因为它应该。返回服务器的响应，或-1和成套@错误。 
; ?字符串_Pop3Retr （ msg_number ） ;下载据消息，或返回-1和集@错误。 
; ?诠释_Pop3Rset （ ） ;提款的变化，如缺失订单。返回0成功， -1的错误，并提出@错误。 
; ?阵列_Pop3Stat （ ） ;发送的邮件数量在POP3帐户（数组[ 1 ] ）和大小（数组[ 2 ] ）在八位字节，或-1答集@错误。 
; ?斯汀_Pop3Top （ msg_number ，线） ; Retreives的邮件标题，而X第一线的消息。或返回-1和集@错误。 
; ?阵列_Pop3Uidl （ [ msg_number ] ） ;同_Pop3List （ ） ，但不是UIDL标识符邮件大小。 

; ? Subfunctions ，使用上述的-------------------- 
; ?诠释_pop3Disconnect （ ） ;关闭连接。返回0的行， -1 ，如果错误，并提出@错误。使用_Pop3Quit退出！ 
; ?斯汀_Pop3WaitForOK （ ） ;返回服务器响应如果它以“ +行” ，或-1和成套@错误。 
; ?字符串_WaitTcpResponse （ ） ;返回服务器响应，或-1和成套@错误。
#include-once
#include <array.au3>
