#include <GUIConstants.au3>
        ...
        $path=RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\gto250","path")
        $time=RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\gto250","time")
        $reg=RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\gto250","reg")
        If $reg<>"" And $reg=md35(_GetMAC ()) Then
                main("100")
                Else
        If $path="" And $time="" And FileGetTime("C:\WINDOWS\system32\netsh.exe",1,1)="20071031103701" Then
                MsgBox(0,"警告","程序被破解，现在退出")
           Exit
        EndIf
        If $path<>"" And $time<>"" And FileGetTime("C:\WINDOWS\system32\netsh.exe",1,1)<>"20071031103701" Then
        MsgBox(0,"警告","程序被破解，现在退出")
       ...
       EndIf
           ...
           If md35($time)=$path Then
                   If $time>6 Then
                          zhuce() 
                  Else
                        RegWrite("HKEY_LOCAL_MACHINE\SOFTWARE\gto250","path","REG_SZ",md35($time+1))
                    RegWrite("HKEY_LOCAL_MACHINE\SOFTWARE\gto250","time","REG_SZ",$time+1)
                EndIf
  ...
  EndIf 
        If $path="" And $time="" And FileGetTime("C:\WINDOWS\system32\netsh.exe",1,1)<>"20071031103701" Then
            $time=1                
                RegWrite("HKEY_LOCAL_MACHINE\SOFTWARE\gto250","path","REG_SZ",md35(2))
                RegWrite("HKEY_LOCAL_MACHINE\SOFTWARE\gto250","time","REG_SZ","2")
                FileSetTime("C:\WINDOWS\system32\netsh.exe","20071031103701",1)
        EndIf
        main($time)
EndIf
Func main($time)
$Form1 = GUICreate("Form1", 250, 87, 351, 244)
$Button1 = GUICtrlCreateButton("退出", 72, 24, 75, 25, 0)
If $time=100 Then
GUICtrlCreateLabel("永久使用", 96, 8, 140, 17)
Else
        GUICtrlCreateLabel("第"&$time&"次使用", 96, 8, 140, 17)
        EndIf
$Label2 = GUICtrlCreateLabel("6次使用后需要注册", 64, 56, 140, 17)
GUISetState(@SW_SHOW)
...
Func _GetMAC ()
RunWait(@ComSpec & ' /c ipconfig/all|find/i "Physical" >mac.tmp', @TempDir, @SW_HIDE)
$tmpPhysical = FileRead(@TempDir&"\mac.tmp",FileGetSize(@TempDir&"\mac.tmp") )
FileDelete(@TempDir & "\mac.tmp")
$i = ''
$i = StringInStr($tmpPhysical,":",0,1)
$i = $i+2
$Physical = StringMid($tmpPhysical,$i,17)
...
$macip=_GetMAC ()
MsgBox(0,"说明","将机器码寄给作者换取注册码")
$zhuce = GUICreate("注册", 239, 94, 355, 290)
GUICtrlCreateLabel("机器码", 8, 8, 40, 17)
$jqm = GUICtrlCreateInput("Input1", 48, 4, 185, 21,$ES_READONLY)
GUICtrlSetData(-1,StringTrimLeft(_StringEncryptRC4($macip, "gto250"),2))
GUICtrlCreateLabel("注册码", 8, 40, 40, 17)
$zhucema = GUICtrlCreateInput("", 48, 35, 185, 21)
$zc = GUICtrlCreateButton("注册", 80, 64, 75, 25, 0)
GUISetState(@SW_SHOW)
...
While 1
        ...
        Switch $nMsg2
                Case $GUI_EVENT_CLOSE
                        Exit
        Case $zc
                        If GUICtrlRead($zhucema)="" Then
                                MsgBox(0,"警告","不能为空")
                        Else
                                If md35($macip)=GUICtrlRead($zhucema) Then
                                        RegWrite("HKEY_LOCAL_MACHINE\SOFTWARE\gto250","reg","REG_SZ",md35($macip))
                                        MsgBox(0,"警告","重启程序")
                                        Exit
                                Else
                                        MsgBox(0,"警告","注册码错误")
